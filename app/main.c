...
void DisposeDocument(DocumentRecord *doc) {
  if (doc) {
    ...
    if (doc->markdownText)
      DisposeHandle(doc->markdownText);
    if (doc->writerText)
      DisposeHandle(doc->writerText);
    if (doc->writerOpsH)
      DisposeHandle(doc->writerOpsH);
    if (doc->lineOffsetsH)
      DisposeHandle(doc->lineOffsetsH);
    if (doc->te)
      WEDispose(doc->te);
    if (doc->hiddenTE)
      WEDispose(doc->hiddenTE);
    DisposePtr((Ptr)doc);
  }
}
...
Boolean gScrollbarDriven = false; /* set during scrollbar-driven loads to
                                     suppress caret-based window shifting */
...
*/
void UpdateMenuBarLook(void) { DrawMenuBar(); }
...
static void MakeMenu(void) {
  ...
  AppendMenu(gViewMenu, "\pMarkdown;Writer;(-;Refresh/R;(-;Zoom In/=;Zoom Out/-;Default "
                        "Size/0;(-;Serif;Sans Serif;(-;Show Status Bar");
...
void MakeWindow(void) {
  ...
  gWindow = NewCWindow(NULL, &bounds, "\p", true, plainDBox, (WindowPtr)-1L,
                       false, 0);
#endif
  ...
  sbRect.bottom += 1;
  gScrollBar =
      NewControl(gWindow, &sbRect, "\p", false, 0, 0, 0, scrollBarProc, 0);

  {
    ...
    gJumpToTopBtn =
        NewControl(gWindow, &btnRect, "\pTop", true, 0, 0, 0, pushButProc, 0);

    /* Jump to End Button (Top Right) */
    btnRect.top = 2;
    btnRect.bottom = 22;
    btnRect.right = viewRect.right;
    btnRect.left = btnRect.right - 50;
    gJumpToEndBtn =
        NewControl(gWindow, &btnRect, "\pEnd", true, 0, 0, 0, pushButProc, 0);
  }
}
...
static void UpdateStatusBar(WindowPtr w, Boolean forceDraw) {
  ...
  sprintf(statusStr, "[%s]    Chars: %ld    Line: %d    Col: %d", modeStr,
          chars, line, col);
  ...
  {
    short fontNum = 0;
    GetFNum("\pTimes", &fontNum); /* Default is Times based on earlier logic, or
                                     use system default */
    TextFont(fontNum);
    TextSize(12);
  }
...
static void DrawTopMiddleButtons(WindowPtr w) {
  ...
  /* 1. Draw Save Button (3.5" Floppy Disk Icon) */
  SetRect(&r, startX, 2, startX + 30, 22);
  EraseRoundRect(&r, 6, 6);
  FrameRoundRect(&r, 6, 6);
  {
    short iconLeft = r.left + (r.right - r.left - 16) / 2;
    short iconTop = r.top + (r.bottom - r.top - 14) / 2;
    PenNormal();
    PenSize(1, 1);

    /* Floppy disk outer body */
    Rect bodyR;
    SetRect(&bodyR, iconLeft, iconTop, iconLeft + 16, iconTop + 14);
    FrameRect(&bodyR);

    /* Top metal slider / shutter door */
    Rect shutterR;
    SetRect(&shutterR, iconLeft + 3, iconTop, iconLeft + 11, iconTop + 6);
    FrameRect(&shutterR);
    MoveTo(iconLeft + 5, iconTop + 2);
    LineTo(iconLeft + 5, iconTop + 5);

    /* Bottom paper label */
    Rect labelR;
    SetRect(&labelR, iconLeft + 3, iconTop + 7, iconLeft + 13, iconTop + 14);
    FrameRect(&labelR);
  }
  if (gDirty)
    InvertRoundRect(&r, 6, 6);

  Boolean isBoldAtCaret = false;
  Boolean isItalicAtCaret = false;
  if (gActiveTE != NULL && (*gActiveTE)->te != NULL) {
    TEHandle te = (*gActiveTE)->te;
    long selStart = (**te).selStart;
    long teLen = (**te).teLength;
    if (teLen > 0) {
      short checkOffset = (short)selStart;
      if (checkOffset > 0 && (**te).selStart == (**te).selEnd) {
        checkOffset = (short)(selStart - 1);
      }
      if (checkOffset >= teLen)
        checkOffset = teLen - 1;
      if (checkOffset < 0)
        checkOffset = 0;
      TextStyle ts;
      short dlh = 0, dfa = 0;
      TEGetStyle(checkOffset, &ts, &dlh, &dfa, te);
      if (ts.tsFace & bold)
        isBoldAtCaret = true;
      if (ts.tsFace & italic)
        isItalicAtCaret = true;
    }
  }
  ...
  /* 4. Draw View Button (24x12 Vector Icon) */
  SetRect(&r, startX + 98, 2, startX + 128, 22);
  EraseRoundRect(&r, 6, 6);
  FrameRoundRect(&r, 6, 6);
  {
    short iconLeft = r.left + (r.right - r.left - 24) / 2;
    short iconTop = r.top + (r.bottom - r.top - 12) / 2;
    PenNormal();
    PenSize(1, 1);

    if (gHideMarkdown) {
      /* Draw MD Icon: 'M' + Down Arrow (24x12 px) */
      MoveTo(iconLeft + 1, iconTop + 11);
      LineTo(iconLeft + 1, iconTop + 2);
      LineTo(iconLeft + 5, iconTop + 8);
      LineTo(iconLeft + 9, iconTop + 2);
      LineTo(iconLeft + 9, iconTop + 11);

      /* Down Arrow */
      MoveTo(iconLeft + 17, iconTop + 2);
      LineTo(iconLeft + 17, iconTop + 10);
      LineTo(iconLeft + 14, iconTop + 7);
      MoveTo(iconLeft + 17, iconTop + 10);
      LineTo(iconLeft + 20, iconTop + 7);
    } else {
      /* Draw Writer Icon: Pencil (24x12 px) */
      MoveTo(iconLeft + 18, iconTop + 1);
      LineTo(iconLeft + 22, iconTop + 5);
      LineTo(iconLeft + 9, iconTop + 11);
      LineTo(iconLeft + 5, iconTop + 7);
      LineTo(iconLeft + 18, iconTop + 1);

      MoveTo(iconLeft + 16, iconTop + 3);
      LineTo(iconLeft + 20, iconTop + 7);

      MoveTo(iconLeft + 5, iconTop + 7);
      LineTo(iconLeft + 1, iconTop + 11);
      LineTo(iconLeft + 9, iconTop + 11);
    }
  }

  /* 5. Draw Refresh Button (Circled Arrow Icon) */
  ...
  FrameRoundRect(&r, 6, 6);
  {
    ...
    /* Arrowhead at 12 o'clock top end pointing clockwise rightward */
    MoveTo(cx + 2, cy - 8);
    LineTo(cx + 5, cy - 5);
    LineTo(cx + 2, cy - 2);
  }
  ...
  if (item == iSearchOK) {
    ...
    if (target[0] > 0) {
      ...
      if (backingH != NULL) {
        ...
        if (FindTextInHandle(backingH, *backingLenPtr, target, currentGlobalPos,
                             &matchStart, &matchEnd)) {
    ...
    if (findTarget[0] > 0) {
      ...
      if (backingH != NULL) {
        ...
        if (item == iReplaceOK) {
          ...
          if (FindTextInHandle(backingH, *backingLenPtr, findTarget,
                               currentGlobalPos, &matchStart, &matchEnd)) {
            PushUndoSnapshot();
              ...
              if (gHideMarkdown && gWriterOpsH != NULL) {
                ...
                for (k = 0; k < gWriterOpCount; k++) {
                  if (ops[k].start >= matchStart + findTarget[0]) {
                    ...
                    ops[k].end += diff;
                    if (ops[k].end < ops[k].start)
                      ops[k].end = ops[k].start;
                  }
                }
                HUnlock(gWriterOpsH);
              }
          ...
          while (searchOffset <= *backingLenPtr - findLen) {
            ...
            HLock(backingH);
            for (j = 0; j < findLen; j++) {
              if (LowerCase((*backingH)[searchOffset + j]) !=
                  LowerCase(findTarget[1 + j])) {
                match = false;
                break;
              }
            }
  ...
  if (menuID == mFile) {
    ...
    if (gHideMarkdown) {
      ...
      case iBulletPoints:
        ...
        break;
      case iNumberedList:
        ApplyLinePrefixHidden("1. ");
        break;
      case iH1:
        ToggleHeadingHidden(1);
        break;
      case iH2:
        ToggleHeadingHidden(2);
        break;
      case iH3:
        ToggleHeadingHidden(3);
        break;
      case iLink:
        DoLinkHidden();
        break;
      case iNone:
        ClearSelectionStyleHidden();
        break;
      }
      InvalidateHeightCache(); /* repaint immediately so style changes show
                                  without scrolling */
    } else {
      switch (menuItem) {
      case iBold:
    ...
    if (WaitNextEvent(everyEvent, &event, 15, NULL)) {
      ...
      case mouseDown:
        ...
        if (part == inSysWindow) {
          ...
          if (newSize != 0) {
            ...
            /* Force a full redraw so centered/right-aligned items don't leave
             * trails */
            InvalRect(&w->portRect);
            EraseRect(
                &w->portRect); /* Explicitly erase now to prevent trails */
            ...
            if (gActiveDoc) {
              ...
              SizeControl(gActiveDoc->scrollBar, sbRect.right - sbRect.left,
          ...
          if (w != FrontWindow()) {
            ...
            if (PtInRect(event.where, &btnSave)) {
              ...
              if (hitControl == gScrollBar) {
                ...
                if (TrackControl(gJumpToTopBtn, event.where, NULL) ==
                    inButton) {
                  HandleJumpToTop();
                }
              } else if (hitControl == gJumpToEndBtn) {
                if (TrackControl(gJumpToEndBtn, event.where, NULL) ==
                    inButton) {
                  HandleJumpToEnd();
                }
              }
      ...
      case autoKey: {
#ifdef ARTFUL_PRO
        SetActiveDocument(GetDocumentForWindow(FrontWindow()));
        if (!gActiveDoc && (event.modifiers & cmdKey) == 0)
          break;
#endif
        ...
        if (keyCode == 0x75) { /* Del (Forward Delete) */
          ...
          handled = true;
        } else if (keyCode == 0x74) { /* Page Up */
          HandlePageUp();
          handled = true;
        } else if (keyCode == 0x79) { /* Page Down */
          HandlePageDown();
          handled = true;
          ...
          handled = true;
                   ...
                   keyCode == 0x7E) { /* Cmd+Cursor Up */
          HandleJumpToTop();
          handled = true;
        } else if ((event.modifiers & cmdKey) &&
                   keyCode == 0x7D) { /* Cmd+Cursor Down */
          HandleJumpToEnd();
          handled = true;
        } else if ((event.modifiers & cmdKey) &&
                   keyCode == 0x7B) { /* Cmd+Cursor Left */
          short lineStart, lineEnd;
          GetCurrentLineRange(&lineStart, &lineEnd);
        ...
        if (handled) {
          ...
          if (isArrowKey && (event.modifiers & shiftKey)) {
            ...
            if (isArrowKey || key == 0x09 || key == 0x0D || key == 0x08 ||
                isContentKey) {
            ...
            if (key == 0x0D) {
              ...
              if (gHideMarkdown) {
                ...
                if (scan < caret && (*hText)[scan] == '-' && scan + 1 < caret &&
                    (*hText)[scan + 1] == ' ') {
                  prefixLen = (scan + 2) - lineStart;
                }
              }
              ...
              if (prefixLen == 0) {
                short i = scan;
                while (i < caret && (*hText)[i] >= '0' && (*hText)[i] <= '9')
                  i++;
              ...
              if (prefixLen == 0) {
                if (gHideMarkdown) {
                  if (scan + 3 < caret && (*hText)[scan] == '[' &&
                      ((*hText)[scan + 1] == ' ' || (*hText)[scan + 1] == 'x' ||
                       (*hText)[scan + 1] == 'X') &&
                      (*hText)[scan + 2] == ']' && (*hText)[scan + 3] == ' ') {
                  ...
                  if (scan + 5 < caret &&
                      ((*hText)[scan] == '-' || (*hText)[scan] == '+' ||
                       (*hText)[scan] == '*') &&
                      (*hText)[scan + 1] == ' ' && (*hText)[scan + 2] == '[' &&
                      ((*hText)[scan + 3] == ' ' || (*hText)[scan + 3] == 'x' ||
                       (*hText)[scan + 3] == 'X') &&
                      (*hText)[scan + 4] == ']' && (*hText)[scan + 5] == ' ') {
              ...
              if (prefixLen > 0) {
                if (lineEnd == lineStart + prefixLen) {
                  ...
                  if (isQuote) {
                    short quoteCount = 0;
                    while (prefixBuf[spaceCount + quoteCount] == '>')
                      quoteCount++;
                    ...
                    if (quoteCount > 1) {
                      ...
                      BlockMove(prefixBuf + spaceCount + 1, newBuf + spaceCount,
                                prefixLen - spaceCount - 1);
                  ...
                  for (i = 0; i < prefixLen; i++) {
                    if (prefixBuf[i] >= '0' && prefixBuf[i] <= '9')
                      isNumbered = true;
                  }
                  if (isNumbered) {
                    long num;
                    ...
                    if (prefixLen > 63)
                      prefixLen = 63;
                    BlockMove(newNumBuf, prefixBuf + numStart,
                              strlen(newNumBuf));
                  }
                }
              }
              ...
              if (doInsertCR) {
                ...
                Boolean wasHeader = ((prevTs.tsFace & bold) &&
                                     prevTs.tsSize > CurrentFontSize());
                ...
                if (prefixLen > 0) {
                  ...
                  if (gHideMarkdown) {
                    if (prefixLen >= 4 && nextPrefix[prefixLen - 4] == '[' &&
                        nextPrefix[prefixLen - 2] == ']') {
                      nextPrefix[prefixLen - 3] = ' ';
                    }
                  } else {
                    if (prefixLen >= 6 && nextPrefix[prefixLen - 4] == '[' &&
                        nextPrefix[prefixLen - 2] == ']') {
                      nextPrefix[prefixLen - 3] = ' ';
                    }
                  }
            ...
            if (isContentKey) {
              ...
              if (caret >= 5) {
                ...
                if (isToday) {
                  ...
                  sprintf(tempBuf, "%04d-%02d-%02d", date.year, date.month,
                          date.day);
          ...
          /* In Markdown view, the line-number gutter is drawn by WEUpdate.
             WEKey only redraws the text area, not the left gutter — so
             invalidate it explicitly after every key so the Mac update
             mechanism redraws it. */
          if (!gHideMarkdown && gActiveTE != NULL && (*gActiveTE)->te != NULL) {
            TEHandle kte = (*gActiveTE)->te;
            Rect gutterRect;
            gutterRect.left = (**kte).viewRect.left;
            gutterRect.right = (**kte).destRect.left;
            gutterRect.top = (**kte).viewRect.top;
            gutterRect.bottom = (**kte).viewRect.bottom;
            InvalRect(&gutterRect);
          }
        }
      ...
      case activateEvt:
        ...
        if (gActiveDoc) {
          if ((event.modifiers & activeFlag) != 0) {
            ...
            WEActivate(gActiveTE);
            InvalRect(
                &gWindow->portRect); /* Force redraw when coming to front */
          } else {
